...
async function finalizeWorkflowExecution(executionId:string, workflowId: string, executionFailed: boolean, creditsConsumed: number) {
    ...
    }).catch((err) => {})  // just keep error in this block, no need to handle it
}
...
async function cleanupEnvironment(environment: Environment){
    if(environment.browser){
        await environment.browser.close().catch(error => console.error("Cannot close broswer : ",error))
    }
}
