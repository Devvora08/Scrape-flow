...
export function NodeInput({input, nodeId} : {input: TaskParam; nodeId: string}) {
    ...
    return <div className={cn("flex justify-start relative p-3 bg-secondary w-full", hasErrors && 'bg-destructive/30')}>
       ...
       {!input.hideHandle && (
          <Handle isConnectable={!isConnected} id={input.name} type="target" position={Position.Left} className={cn("!bg-primary !border-2 !border-background !w-4 !h-4", ColorForHandle[input.type])}/>
       )}
